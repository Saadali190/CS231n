...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (out,gamma,xhat,ivar,eps,sqrt_var,sqr_xmu,xmu)
        ...
        mean_x = x - running_mean

        norm = mean_x / np.sqrt(running_var + eps)

        batch_norm = gamma* norm + beta
...
def batchnorm_backward(dout, cache):
    ...
    out,gamma,xhat,ivar,eps,sqrt_var,sqr_xmu,xmu =cache 
...
def conv_backward_naive(dout, cache):
    ...
    for i, j in combs:
        ...
        if k==0:
          print(dout_slice.shape,dout.shape)
          k+=1
...
def max_pool_forward_naive(x, pool_param):
    ...
    N,C,H,W = x.shape
    Hp,Wp,stride = itemgetter('pool_height','pool_width','stride')(pool_param)
    ...
    for n in range(N):
      for ch in range(C):
        for i in range(Ht):
          ...
          for j in range(Wt):
            col_str = j*stride
            x_slice = x[n,ch,row_str:(row_str+Hp),col_str:(col_str+Wp)]
            out[n,ch,i,j] += np.max(x_slice) 
    ...
    for n in range(N):
      for ch in range(C):
        for i in range(Ht):
          ...
          for j in range(Wt):
            jj = j*stride
            x_slice = x[n,ch,ii:(ii+Hp),jj:(jj+Wp)]
            mx,my= np.unravel_index(x_slice.argmax(), x_slice.shape) 
            dx[n,ch,ii+mx,jj+my] += dout[n,ch,i,j]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    tdout = np.transpose(dout,[0,2,3,1])
    dx,dgamma,dbeta=batchnorm_backward(np.reshape(tdout,(-1,tdout.shape[-1])),cache)
    dx = dx.reshape(tdout.shape).transpose([0,3,1,2])
